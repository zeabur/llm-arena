...
describe('/api/submit', () => {
  ...
  it('returns 401 without token', async () => {
    ...
    const res = await POST({ json: async () => ({ threadID: 't1', result: 'TIE' }), cookies: { get: () => undefined } } as any);
  ...
  it('returns 403 when thread does not belong to user', async () => {
    ...
    const { POST } = await import('../../app/api/submit/route');
    const res = await POST({ json: async () => ({ threadID: '507f1f77bcf86cd799439012', result: 'TIE' }), cookies: { get: () => ({ value: 'token' }) } } as any);
    expect(res.status).toBe(403);
  });

  it('updates result when authorized', async () => {
